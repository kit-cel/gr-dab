...
class DABstep(QtGui.QMainWindow, user_frontend.Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        self.components = [
            ...
            {"label": self.t_label_comp7, "data_rate_label": self.t_label_rate7, "data_rate": self.t_spin_rate7,
             ...
             "src_btn": self.t_btn_path_src7, "src_path": "None", "label_label": self.t_label_label7, "edit_label": self.t_edit_service_label7, "combo_dabplus": self.t_combo_dabplus7, "btn_record": self.t_btn_record7, "audio_rate_label": self.t_label_audio_rate7, "combo_audio_rate": self.t_combo_audio_rate7, "combo_audio_rate_dab": self.t_combo_audio_rate_dab7, "combo_stereo": self.t_combo_stereo7}]
        ...
        if not self.receiver_running:
            ...
            if (not self.src_is_USRP) and (not self.src_is_RTL) and (self.file_path == "None"):
                ...
                self.my_receiver = usrp_dab_rx.usrp_dab_rx(self.spin_dab_mode.value(), self.spinbox_frequency.value(), self.bit_rate, self.address, self.size, self.protection, self.audio_bit_rate, self.dabplus,
                                                  self.src_is_USRP, self.src_is_RTL, self.label_dyn_lab, self.label_mot_image, self.file_path)
    ...
    def play_audio(self):
        ...
        if self.subch is not self.table_mci.currentRow():
            ...
            self.my_receiver = usrp_dab_rx.usrp_dab_rx(self.spin_dab_mode.value(),
                                                               ...
                                                               self.src_is_USRP, self.src_is_RTL, self.label_dyn_lab, self.label_mot_image, self.file_path, prev_src=self.temp_src)
    ...
    def adjust_audio_sampling_rate(self):
        ...
        if new_sampling_rate != self.audio_bit_rate and new_sampling_rate != -1:
            ...
            self.my_receiver = usrp_dab_rx.usrp_dab_rx(self.spin_dab_mode.value(),
                                                       ...
                                                       self.address, self.size,
                                                       self.protection, self.audio_bit_rate, self.dabplus,
                                                       self.src_is_USRP, self.src_is_RTL,
                                                       self.label_dyn_lab, self.label_mot_image,
                                                       self.file_path, prev_src=self.temp_src)
    ...
    def dev_mode_open(self):
        ...
        self.led_msc.setText("<img src=\"" + str(self.path) + "led_green.png\">")
        self.led_msc.show()
        self.label_label_msc.show()
        self.label_firecode.setText("")
        self.label_fic.show()
        self.led_fic.setText("<img src=\"" + str(self.path) + "led_green.png\">")
        self.led_fic.show()
